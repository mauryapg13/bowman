...
| 38 | **R8: a scheduled debit joins a bill stream only when its amount is the bill's amount / inside the recorded range; otherwise it is an extra confirmed payment and the bill is still forecast** (credits unchanged) | 88 % | 119,187 | yes | request_24 +2,673 → +163 (the dropped 2,510 insurance policy payment); 0 rows broken; mean rel safe error 13.4 → 12.7 %. On the 250: 44, 84, 104, 224 change; the changed stream view re-read 6 messages live (cache committed, final run offline). `scratch/ruleset_progress.md` 14:50 |
| 39 | `earliest_date_for_full_payment` as a point check (balance on that day − requested ≥ floor, no look-ahead) and look-ahead windows K ∈ {30, 45, 60, 75} after the payment day, each with horizon 84 and 90 | 13–22 | — | **rejected** | point check: earliest 23 → 11/25, categoricals 22 → 17; look-ahead: ≤ current everywhere (h84 K=60/75 equal, never better). Motivation was request_10, whose key outflow is exactly one more groceries + transport + dining (25,666 vs 25,612 residual) — the items our 90-day curve puts on day 89 — while 08/12/13 need the day-87 bill excluded; no single window satisfies both. `scratch/fit_pointcheck.py` |
